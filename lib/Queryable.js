...
_.extend(queryableProto, {
	...
	replaceWith: function ( replacement ) {
		...
		_.each(this, function (datum) {
			var dataPointIndex = queryable._getDataPointIndex(datum),
				isBuilder = replacement._isNodeBuilder;
			
			if (isBuilder) {
				replacement = replacement.build();
			}
			
			datum.parentArray.splice(dataPointIndex, 1, replacement);
		});
	...
	createNewNode: function (name) {
		var newNode = NodeBuilder.create(name),
			queryable = this;
		
		_.extend(newNode, {
			replaceQuery: function () {
				return queryable.replaceWith( newNode );
			}
		});
